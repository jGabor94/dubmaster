...
export async function SiteHeader({ active, showNavigation = true }: SiteHeaderProps) {
  ...
  return (
    <header className="relative z-10 mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-6 lg:px-10">
      <Link href={homeHref} className="flex shrink-0 items-center gap-3" aria-label="Dubmaster kezdőlap">
      ...
      <div className="flex items-center gap-7">
        {showNavigation && <nav className="hidden items-center gap-7 text-sm font-medium text-white/60 sm:flex" aria-label="Fő navigáció">
          <Link href={homeHref} className={`transition-colors hover:text-white ${active === "home" ? "text-white" : ""}`}>Kezdőlap</Link>
